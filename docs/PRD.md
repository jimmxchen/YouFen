...
有份是一个无代码社群共治网站，帮助社群运营者把成员的参与和贡献转化为"发言权"，让成员可以共同参与社区投票、规则共创和重要决策。AI 负责生成规则和总结贡献，Injective 负责记录关键投票与规则版本。
...
> 有份把 Injective 带给非 Web3 社群运营者。普通用户不需要理解区块链，不需要连接钱包，也不需要支付 gas 费。但关键的社区发言权分配、规则版本变更和投票结果，都可以通过 Injective 生成公开可信记录。
>
...
> 有份本身就是在 AdventureX 社区中公开构建的产品。我们不只是"发动态"，而是用自己的产品让用户投票决定产品下一步要做什么。
>
...
> 有份是一个无代码社群共治网站，帮助社群运营者把成员的真实参与转化为发言权。成员不再只是群里的旁观者，而是可以用自己贡献获得的话语权参与社区决策。
>
> AI 帮运营者生成参与规则、识别贡献和总结议题，Injective 在后台为重要投票和规则变化生成公开可信记录。
>
> 我们想解决的问题很简单：让每个真正参与社区的人，都能在社区里真的有份。
