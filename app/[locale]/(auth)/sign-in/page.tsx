...
export default function SignInPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("")
    setLoading(true)
    const result = await signIn(isZh ? phone : email, isZh ? "sms-placeholder" : password)
    setLoading(false)
    if (result.error) {
        ...
        <div className="w-full flex items-center justify-center p-6">
          <div className="w-full max-w-md bg-white rounded-3xl shadow-sm border border-gray-200 px-8 py-10">
            <form onSubmit={handleSubmit} className="flex flex-col items-center">
              ...
              <button
                type="button"
                className="w-full mt-7 bg-gray-100 hover:bg-gray-200/80 flex items-center justify-center h-12 rounded-full transition-colors"
              >
                {isZh ? (
                  <>
                    <WeChatIcon />
